...
export default function NotFound() {
    ...
    return (
        <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden main-bg-color px-4 text-slate-900">
            {/* 🔵 Animated background */}
            <div className="absolute inset-0 z-0">
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                    <div
                        key={i}
                        className="absolute rounded-full border border-slate-900/10 opacity-70"
                        style={{
                            width: `${(i + 1) * 100}px`,
                            height: `${(i + 1) * 100}px`,
            ...
            `,
                ...
                <p className="mb-8 max-w-md text-slate-800">
              ...
              `,
                    ...
                    <Button
                        variant="outline"
                        onClick={() => router.back()}
                        className="group relative overflow-hidden bg-white/50 border-slate-900 px-6 py-2 text-slate-900 transition-all hover:border-slate-900 hover:text-slate-900 flex items-center justify-center"
                        style={{
                            transform: `
                translate(${mousePosition.x * -5}px, ${mousePosition.y * -5}px)
              `,
