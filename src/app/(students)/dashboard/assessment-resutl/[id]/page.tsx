...
interface EmailExerciseResponse {
  success: boolean
  message?: string
  data?: EmailExerciseResult
}
...
export default function AssessmentResultPage() {
  ...
  const { data, isLoading, isError, error } = useQuery<EmailExerciseResponse>({
    queryKey: ['intray-email-result', taskId],
    enabled: Boolean(taskId && token),
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/intrayemail/${taskId}`,
        {
          method: 'GET',
          headers: {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading assessment result...</p>
        </div>
      </div>
    )
  }

  if (isError || !result || data?.success === false) {
    const message =
  ...
  const overallGradePercent = gradeToPercent(overallGrade)

  const feedbackItems = [
    { label: 'Context Understanding', value: contextUnderstanding },
          ...
          <div className="flex items-start justify-between mb-8">
            <div className="space-y-2">
              ...
              <p className="text-[#4A5565] font-normal font-sm md:text-base">
                Great job! Here's how you performed in this email exercise.
              </p>
            </div>
            <button
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                ...
                <h2 className="text-base font-medium text-white">
                  {aiassigmentId.title || 'In-Tray Email Exercise'}
                </h2>
              </div>
            </div>

            <div className="bg-[#27272A] border border-[#3F3F46] rounded-[12px] py-2 px-4 text-right flex">
              <p className="text-xl md:text-2xl font-bold text-white">{completionRate ?? 0} </p>
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                ...
                <h2 className="text-sm font-medium text-[#71717A]">
                  {aiassigmentId.type?.replace('_', ' ') || 'Email Exercise'}
                </h2>
              </div>
            </div>
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold text-[#18181B]">{typeSpeed ?? 0}</span>
                </div>
                <div className="w-full bg-[#E4E4E7] rounded-full h-3">
                  <div
                    className="bg-[#EAB308] h-3 rounded-full transition-all"
                    style={{ width: `${clampPercent(typeSpeed)}%` }}
                  />
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold text-[#18181B]">{overallGrade || ''}</span>
                </div>
                <div className="w-full bg-[#E4E4E7] rounded-full h-3">
                  <div
                    className="bg-[#EAB308] h-3 rounded-full transition-all"
                  ...
                  <ul className="space-y-3">
                    {feedbackItems.map((item) => (
                      <li key={item.label} className="flex items-start gap-3">
                        <span className="text-sm font-semibold text-[#111827] min-w-[160px]">
                          {item.label}
                        </span>
                        <span className="text-[#374151] leading-relaxed">{item.value}</span>
                      </li>
                    ))}
                  </ul>
                </section>
